...
const plans = [
  {
    ...
    highlight: false,
    whatsAppMessage:
      "Hola, quiero registrar mi negocio con el Plan Básico Gratuito en IztapaMarket.",
  ...
  },
  {
    ...
    highlight: true,
    whatsAppMessage:
      "Hola, estoy interesado en el Plan Destacado de $99 MXN para mi negocio en IztapaMarket.",
  ...
  },
  {
    ...
    priceSuffix: "MXN / mes",
    description:
      "La solución completa para el crecimiento acelerado de tu negocio.",
    features: [
    ...
    highlight: false,
    whatsAppMessage:
      "Hola, me interesa el Plan Premium de $199 MXN para mi negocio en IztapaMarket.",
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      >
        <img
          ...
          src={pageHeaderImageUrl}
        />
        <h1 className="text-4xl md:text-5xl font-extrabold text-secondary mb-4">
          Haz Crecer tu Negocio en Iztapalapa
        </h1>
        <p className="text-lg md:text-xl text-foreground/80 max-w-2xl mx-auto">
          Elige el plan perfecto para conectar con miles de clientes locales.
          ¡Más visibilidad, más ventas! Ofrecemos opciones para cada necesidad y
          presupuesto.
        </p>
      </motion.section>
        ...
        {plans.map((plan, index) => (
          <motion.div
            ...
            className={`flex flex-col ${
              plan.highlight ? "transform lg:scale-105 z-10" : ""
            }`}
          >
            <Card
              className={`shadow-xl hover:shadow-2xl transition-shadow duration-300 border-2 ${plan.borderColor} flex flex-col flex-grow ${plan.bgColor} ${plan.textColor}`}
            >
              <CardHeader className={`p-6 ${plan.highlight ? "relative" : ""}`}>
                ...
                <CardDescription
                  className={`text-center text-5xl font-extrabold mt-4 ${
                ...
                <ul className="space-y-2.5 text-sm">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start">
                      <CheckCircle
                        className={`h-5 w-5 mr-2.5 mt-0.5 flex-shrink-0 ${
      ...
      >
        <h2 className="text-2xl md:text-3xl font-bold text-secondary mb-6">
          ¿Tienes Preguntas o Necesitas un Plan a Medida?
        </h2>
        <p className="text-foreground/70 mb-8 max-w-lg mx-auto">
          Estamos aquí para ayudarte. Contáctanos y diseñaremos la mejor
          estrategia para tu negocio en Iztapalapa. Queremos ser tu aliado para
          crecer.
        </p>
        <Button
          asChild
          size="lg"
          className="bg-[#25D366] hover:bg-[#1ebe5d] text-white font-bold py-3 px-6 rounded-lg shadow-md text-lg transition-colors duration-300 ease-in-out"
        >
          <a
            href={`https://wa.me/${officialWhatsAppNumber}?text=${encodeURIComponent(
              "Hola, tengo algunas preguntas sobre los planes de IztapaMarket."
            )}`}
            target="_blank"
            rel="noopener noreferrer"
      ...
      >
        <h2 className="text-xl font-semibold text-secondary mb-3">
          ¿Listo para empezar pero no sabes cómo registrarte?
        </h2>
        <p className="text-foreground/70 mb-5">
          Visita nuestra página de registro para añadir tu negocio al
          directorio. Si tienes un plan de pago, te guiaremos después de
        ...
        >
          <Link
            to="/registrarse?admin=iztapasecreto2025"
            aria-label="Ir a la página de registro de negocios en IztapaMarket"
          >
