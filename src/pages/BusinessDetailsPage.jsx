...
import {
  ...
  formatBusinessData,
} from "@/services/businessService.jsx";
...
const BusinessDetailPage = () => {
  ...
  const loadBusinessDetails = useCallback(async () => {
    ...
    try {
      ...
      const categoryDetails = allCategories.find(
        (c) =>
          c.dbName?.toLowerCase().trim() ===
            rawBusinessData.categoria?.toLowerCase().trim() ||
          c.slug?.toLowerCase().trim() ===
            rawBusinessData.categoria?.toLowerCase().trim() ||
          c.name?.toLowerCase().trim() ===
            rawBusinessData.categoria?.toLowerCase().trim()
      );
      const formattedBusiness = formatBusinessData(
        rawBusinessData,
        categoryDetails?.name
      );

      const mapEmbedUrlWithKey =
        rawBusinessData.mapa_embed_url ||
        (rawBusinessData.direccion
          ? `https://www.google.com/maps/embed/v1/place?key=${
              import.meta.env.VITE_GOOGLE_MAPS_API_KEY ||
              "YOUR_GOOGLE_MAPS_API_KEY_FALLBACK"
            }&q=${encodeURIComponent(rawBusinessData.direccion)}`
          : null);
      ...
      if (formattedBusiness.category) {
        const rawRelatedData = await fetchRelatedBusinesses(
          formattedBusiness.category,
          formattedBusiness.id,
          3
        );
        const formattedRelated = rawRelatedData.map((b) => {
          const relCategoryDetails = allCategories.find(
            (c) =>
              c.dbName?.toLowerCase().trim() ===
      ...
      toast({
        ...
        description:
          error.message ||
          "No se pudo encontrar el negocio solicitado o ocurrió un error.",
        variant: "destructive",
      });
      setBusiness(null);
    } finally {
  ...
  const seoDescription = business.description
    ...
    : `Detalles de ${business.name}, un negocio de ${business.categoryDisplay} en Iztapalapa. Encuentra dirección, contacto y más.`;
  const canonicalUrl = `${
    import.meta.env.VITE_APP_BASE_URL || "https://iztapamarket.com"
  }${location.pathname}`;
    ...
    >
      ...
      >
            ...
            <Link
              to={
                business.categorySlug
                  ? `/categorias/${business.categorySlug}`
                  : "/categorias"
              }
              className="text-primary hover:text-primary/80"
            >
              <ChevronLeft className="mr-2 h-4 w-4" /> Volver a{" "}
              {business.categoryDisplay || "Categorías"}
            </Link>
          </Button>
        </div>
          ...
          <CardContent className="p-6 md:p-8 grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-6">
              ...
              <BusinessVideoSection
                videoEmbedUrl={business.videoEmbedUrl}
        ...
        <RelatedBusinessesSection
          ...
          currentCategory={business.categoryDisplay}
        />
      </motion.div>
    </SeoManager>
  );
